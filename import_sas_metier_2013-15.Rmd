---
title: "Matching SAS data 2013-15"
author:
- Perttu Rantanen
- Petri Sarvamaa
- Mira Sustar
...
pacman::p_load(tidyverse, here, patchwork, RPostgres, openxlsx, janitor, haven, labelled)
...
codelist <- icesVocab::getCodeList("Metier6_FishingActivity", date = NULL)
```

## Data extraction from SAS

! NO NEED TO RUN THIS CODE ! This was originally saved to DB in 2025.

### get metier data

Data saved to DCPROD: sas_logbook_raw_2013_15
...
# DO NOT RUN
#' #################################
# copy data to project orig folder
...
invisible(write.dbTable(dbname = 'kake_siirto', "vault", "sas_logbook_raw_2013_15", metier_2013_15, overwrite = FALSE))
```
...
# SAVEPOINT -------- ####
# uncomment and save to DB if renewed
invisible(write.dbTable(dbname = 'kake_siirto', "vault", "sas_shorelogs_raw_2013_15", shore_2013_15, overwrite = FALSE))
```
...
* rerun 2025 raw data from DCPROD database

```{r add-metier}
#| eval: true

# get 2025 prep data from DB
metier_2013_15 <- read.dbTable(schema="vault", table="sas_logbook_raw_2013_15", dbname = "kake_siirto")

metier_2013_15 |> count(KALASTUSVUOSI, VESSEL_LENGTH) |> 
   pivot_wider(names_from = VESSEL_LENGTH, values_from = n,
               names_expand = TRUE,  names_sort = FALSE) |>
   flextable::flextable() |>
   flextable::colformat_num(j="KALASTUSVUOSI", big.mark="") |>
   flextable::set_caption("Check raw metier_2013_15 VESSEL_LENGTH classes")

shore_2013_15 <- read.dbTable(schema="2025-04-10-dcprod", table="sas_shore_raw_2013_15", dbname = "kake_siirto")

shore_2013_15 |> count(KALASTUSVUOSI, VESSEL_LENGTH) |> 
   pivot_wider(names_from = VESSEL_LENGTH, values_from = n,
               names_expand = TRUE,  names_sort = FALSE) |>
   flextable::flextable() |>
   flextable::colformat_num(j="KALASTUSVUOSI", big.mark="") |>
   flextable::set_caption("Check raw shore_2013_15 VESSEL_LENGTH classes")
...
## correct vessel length

```{r correct-vessel-length}
#| eval: true
...
# check vessel length classes and VLENGTH_FDI
...
# checking records
logbook_13_15 |> count(KALASTUSVUOSI, length_orig) |> rename(n_hauls=n) |> flextable::flextable() |> 
  flextable::colformat_num(j = "KALASTUSVUOSI", big.mark = "") |> 
  flextable::set_caption("Check length_orig classes in logbook_13_15")

logbook_13_15 |> count(KALASTUSVUOSI, VESSEL_LENGTH) |> rename(n_hauls=n) |> flextable::flextable() |>
    flextable::colformat_num(j = "KALASTUSVUOSI", big.mark = "") |> 
  flextable::set_caption("Check VESSEL_LENGTH classes in logbook_13_15")

# compare now with PITUUS (real length of vessel)
logbook_13_15 <- logbook_13_15 |>
                    mutate(test = case_when(
                      between(PITUUS,0,799)  ~ "VL0008",
                      between(PITUUS,800,1199) ~ "VL0812",
                      between(PITUUS, 1200, 1799) ~ "VL1218",
                      between(PITUUS, 1800, 2399) ~ "VL1824",
                      between(PITUUS, 2400, 3999) ~ "VL2440",
                      between(PITUUS, 3999, 10000) ~ "VL40XX",
                      .default = as.character(PITUUS)
                    ))

logbook_13_15 |> count(test, VESSEL_LENGTH) |> flextable::flextable() |> 
  flextable::set_caption("Check vessel length derived from PITUUS (test) and VESSEL_LENGTH")
```
...
# checking records
shorelogs_13_15 |> count(KALASTUSVUOSI, length_orig) |> rename(n_recs=n) |> flextable::flextable() |> 
  flextable::colformat_num(j = "KALASTUSVUOSI", big.mark = "") |> 
  flextable::set_caption("Check length_orig classes in shorelogs_13_15")

shorelogs_13_15 |> count(KALASTUSVUOSI, VESSEL_LENGTH) |> rename(n_recs=n) |> flextable::flextable() |> 
  flextable::colformat_num(j = "KALASTUSVUOSI", big.mark = "") |> 
  flextable::set_caption("Check VESSEL_LENGTH classes in shorelogs_13_15")

# ok this needs fixing! (2026)

shorelogs_13_15 <- shorelogs_13_15 |>
                    mutate(VESSEL_LENGTH = case_when(
                      VESSEL_LENGTH == "VL0010" ~ "VL0008",
                      VESSEL_LENGTH == "NK" ~ "VL0008",
                      .default = VESSEL_LENGTH
                    ))

shorelogs_13_15 |> count(KALASTUSVUOSI, VESSEL_LENGTH) |> rename(n_recs=n) |> flextable::flextable() |> 
  flextable::colformat_num(j = "KALASTUSVUOSI", big.mark = "") |> 
  flextable::set_caption("Check VESSEL_LENGTH classes again - VL0010 should now be VL0008")
...
shorelogs_13_15 |> count(test, VESSEL_LENGTH) |> flextable::flextable() |> 
  flextable::set_caption("Check vessel length derived from PITUUS (test) and VESSEL_LENGTH")

shorelogs_13_15 <- shorelogs_13_15 |>
                    mutate(VESSEL_LENGTH = case_when(
                      test == "VL0812" & VESSEL_LENGTH == "VL0008" ~ "VL0812",
                      .default = VESSEL_LENGTH))

shorelogs_13_15 |> count(test, VESSEL_LENGTH) |> flextable::flextable() |> 
  flextable::set_caption("Check vessel length derived from PITUUS (test) and VESSEL_LENGTH again")

shorelogs_13_15 |> count(YEAR, VESSEL_LENGTH) |> flextable::flextable() |> 
  flextable::set_caption("Check vessel length derived from PITUUS (test) and VESSEL_LENGTH again")

# drop test column
shorelogs_13_15 <- shorelogs_13_15 |> select(-test)
```
...
active.passive |> flextable::flextable() |> flextable::autofit() |> flextable::set_caption("METIER4 lookup for active and passive gears")
...
mesh.sizes |> flextable::flextable() |> flextable::autofit() |> flextable::hline(i=6) |> flextable::set_caption("Mesh size ranges for Baltic")
...
mesh.lookup |> flextable::flextable() |> flextable::autofit() |> flextable::set_caption("all possible mesh size combinations")
...
# invisible(write.dbTable(dbname = 'kake_siirto', "vault", "mesh_sizes_DC2024", mesh.lookup, overwrite = FALSE))
# added to DB by Perttu 19-06-2025
# readded to vault by Joanne 16/06/2026
```

## logbook mesh sizes

```{r logbook-mesh-sizes}
#| eval: true
...
logbook_13_15 |> count(metier6_orig) |> flextable::flextable() |> flextable::set_caption("before cleaning")                    
logbook_13_15 |> count(METIER6) |> flextable::flextable() |> flextable::autofit() |> flextable::set_caption("after cleaning") 
```

## shorelogs mesh sizes

```{r shorelogs-mesh-sizes}
#| eval: true
...
shorelogs_13_15 |> count(metier6_orig) |> flextable::flextable() |> flextable::set_caption("before cleaning")                    
shorelogs_13_15 |> count(METIER6) |> flextable::flextable() |> flextable::autofit() |> flextable::set_caption("after cleaning") 

# create reasonable lookup for meshsizes based on metier and species and impute meshsize
...
ewkb_to_sf <- function(data) {
  return(sf::st_as_sfc(structure(data, class="WKB"), EWKB=T))
}
...
icesRectangle <- sf::st_as_sf(icesRectangle) |> sf::st_transform(epsg = 4326)
...
logbook_13_15 <- logbook_13_15 |> 
  left_join(icesRectangle |> 
              select(rktl_name, ices_name, area_code_full) |> sf::st_drop_geometry(), 
...
# testing
logbook_13_15 |> count(area_code_full,ices_name) |> flextable::flextable()
shorelogs_13_15 |> count(area_code_full,ices_name) |> flextable::flextable()
...
# SAVEPOINT -------- ####
# uncomment and save to DB if renewed
invisible(write.dbTable(dbname = "kake_siirto", dcprodschema, "species_lookup", species_lookup, overwrite = FALSE))
```
...
# need to check why fecR fails do deal with these records !!!
# if NA's in KALASTUSPAIVAT/MERIPAIVAT
LOGBOOK <- LOGBOOK |>
              mutate(KALASTUSPAIVAT = case_when(
                      is.na(KALASTUSPAIVAT) ~ PYYNTIPV,
                      TRUE ~ KALASTUSPAIVAT),
                     MERIPAIVAT = case_when(
                      is.na(MERIPAIVAT) ~ PYYNTIPV,
                      TRUE ~ MERIPAIVAT))

# SAVEPOINT -------- ####
# uncomment and save to DB if renewed
invisible(write.dbTable(dbname = 'kake_siirto', "vault", "sas_logbook_clean_2013_15", LOGBOOK, overwrite = TRUE))

```

## fishing, seadays fec.R - shore 
...
# SAVEPOINT -------- ####
# uncomment and save to DB if renewed
invisible(write.dbTable(dbname = 'kake_siirto', "vault", "sas_shorelogs_clean_2013_15", shorelogs_13_15, overwrite = TRUE))
```


## join datasets and save for table J 
...
LOG <- LOGBOOK |> select(ALUS, ALUSNIMI, KKERTA_TUNNUS=KALASTUSKERTATUNNUS, PURKUPVM, PYYDYS, PYYDLKM, 
                         ...
                         KMATKA_TUNNUS, TRIP_ID=trip_id, ID, MERIPAIVAT, KALASTUSPAIVAT, LOMAKE)
...
# now fix FROM and TO for imputed MESH SIZES
KALASTUSAKTIVITEETTI_13_15 <- KALASTUSAKTIVITEETTI_13_15 |>
...
# SAVEPOINT -------- ####
# uncomment and save to DB if renewed
# invisible(write.dbTable(dbname = 'kake_siirto', "vault", "logbook_13_15_for_J", logbook_13_15_for_J, overwrite = TRUE))
```

## add fish pricing data 
...
# dim(metier.fish)
# check records
metier.fish |> count(SPECIES) |> flextable::flextable()
...
metier.fish |> filter(is.na(VALUE)) |> count(YEAR,ICES, SPECIES) |>
  adorn_totals("row") |> flextable::flextable() |> 
  flextable::set_caption("fish species without commercial value in certain year")
...
# SAVEPOINT -------- ####
# uncomment and save to DB if renewed
invisible(write.dbTable(dbname = 'kake_siirto', "vault", "sas_combined_clean_2013_15", KALASTUSAKTIVITEETTI_13_15, overwrite = TRUE))
