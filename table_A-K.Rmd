...
zip_file <- list.files(
  path_IC_ORIG,
  pattern = paste0(run.year-1, ".*\\.zip$"),
  full.names = TRUE
)

unzip(zip_file, exdir = last_path)

IC_SD <- read.csv(list.files(last_path,          
                     pattern = " SD .*\\.csv$", 
                     full.names=TRUE))

IC_SI <- read.csv(list.files(last_path, 
                     pattern = " SI .*\\.csv$", 
                     full.names=TRUE))

IC_HI<- read.csv(list.files(last_path, 
                     pattern = " HI .*\\.csv$", 
                     full.names=TRUE))
...
#save to der folder
saveRDS(IC_DB, file = file.path(last_path,"IC_DB.rds"))
