...
knitr::opts_chunk$set(echo = TRUE, eval=TRUE, warnings=FALSE, errors=FALSE)
rStudio=TRUE
```
...
kap2  |> count(YEAR, VESSEL_LENGTH) |> pivot_wider(names_from = VESSEL_LENGTH, values_from=n) |> flextable::flextable() |> flextable::set_caption("Vessel lengths from capacity table")
...
# 
table_A |> head() |> flextable::flextable() |>
  flextable::fontsize(size = 3, part = "all") |>
  flextable::rotate(j = colnames(table_A), align = "bottom", rotation = "tbrl", part = "header") |>
  flextable::autofit() |>
  flextable::theme_zebra() |>
  flextable::colformat_num(j = "YEAR", big.mark = "") |>
  flextable::set_caption( "Tiny preview TABLE A-full")
...
flextable::save_as_image(ft, path = file.path(path_out,"table_B_preview.png"))
...
- removed, as we don't have age information
...
flextable::save_as_image(ft, path = file.path(path_out,"table_C_preview.png"))
...
knitr::include_graphics("table_C_preview_rotated.png")
...
flextable::save_as_image(ft, path = file.path(path_out,"table_D_preview.png"))
...
flextable::save_as_image(ft, path = file.path(path_out,"table_E_preview.png"))
...
E_domains <- table_E_mega |> ungroup() |> select(YEAR, DOMAIN_LANDINGS)
...
table_F2 |> filter(is.na(DOMAIN_CHECK)) |> 
  ...
  flextable::set_caption("DOMAIN_LANDINS, that are not reported in table E") |> 
  flextable::colformat_num(j="YEAR", big.mark = "")
...
flextable::save_as_image(ft, path = file.path(path_out,"table_F_preview.png"))
...
full_join(
  ...
  ) |> flextable::flextable() |> flextable::autofit() |> flextable::set_caption("Remaining DOMAIN_DISCARDS after tagging tagging table C+D") |> flextable::colformat_num(j="YEAR", big.mark = "")
...
full_join(
  ...
  ) |> flextable::flextable() |> flextable::autofit() |> flextable::set_caption("Remaining DOMAIN_LANDINGS after tagging tagging table E+F") |> flextable::colformat_num(j="YEAR", big.mark = "")
...
IC_DB |> filter(Year == 2021 & Season %in% c(1,2)) |> View()
...
flextable::save_as_image(ft, path = file.path(path_out,"table_A_preview.png"))
...
flextable::save_as_image(ft, path = file.path(path_out,"table_G_preview.png"))
...
flextable::save_as_image(ft, path = file.path(path_out,"table_H_preview.png"))
...
flextable::save_as_image(ft, path = file.path(path_out,"table_I_preview.png"))
...
missing_kap |> count(ULKOINENTUNNUS) |> flextable::flextable() |> flextable::set_caption("vessels not registered in capacity table or missing") |> flextable::autofit()
...
extra_kap |> count(ULKOINENTUNNUS) |> flextable::flextable() |> flextable::set_caption("vessels not registered in capacity table") |> flextable::autofit()
...
flextable::save_as_image(ft, path = file.path(path_out,"table_J_preview.png"))
...
5. DISCARDS: Estimated discards in tonnes [precision to 3 digits after the decimal]; missing values not
allowed.
6. DISCARD_CV: the coefficient of variation of the estimate based on the sample available for the
strata considered (i.e., DOMAIN_DISCARDS) and the sampling design. This is calculated for the
weight of discards. Mandatory. NK if not known.
7. DISCARD_CI_UPPER: the upper confidence limit of the estimate based on the strata sampled (i.e.,
DOMAIN_DISCARDS), 95% confidence interval (i.e., the confidence interval that allows us to be
95% confident that the real value is contained into; is between the upper and the lower confidence
limit), supplied in weight. Mandatory. NK if not known.
8. DISCARD_CI_LOWER: the lower confidence limit of the estimate based on the strata sampled (i.e.,
DOMAIN_DISCARDS), 95% confidence interval (i.e., the confidence interval that allows us to be
95% confident that the real value is contained into; is between the upper and the lower confidence
limit), supplied in weight. Mandatory. NK if not known.
9. TOTAL_TRIPS: The total number of trips that relate to domain; a number should only be given only
if it relates to this domain, otherwise use ‘NK’.
10. NO_TRIPS_SAMPLED_WITH_SPECIES: [integer] Number of trips in which the species was
sampled/observed; if not known use ‘NK’.
11. NO_TRIPS_SAMPLED: [integer] Number of trips sampled for the specified domain discards; if not
known use ‘NK’.
```
...
flextable::save_as_image(ft, path = file.path(path_out,"table_K_preview.png"))
