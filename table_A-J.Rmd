...
# one vessel was not in capcity table and VESSEL_LENGTH was overwritten
...
# testing
missing_kap <- j6b |> filter(is.na(PAAKONETEHO)) |> select(ULKOINENTUNNUS, YEAR)
...
j6b <- j6b |>
  ...
  mutate(
    # Estimate vessel "birth/build reference" from any known AGE/YEAR pair
...
# there is one vessel, that originally ahd information in activity table and was flattened in the cleaning against capacity in Chunk 12: clean_capacity
# might need to change the methodology for non-existing vessels here to the top!
# for now, just fix it manually

j6b <- j6b |> mutate(PAAKONETEHO    = if_else(ULKOINENTUNNUS == "FIN-31409-V", 45, PAAKONETEHO),
                    VETOISUUS      = if_else(ULKOINENTUNNUS == "FIN-31409-V", 2.81, VETOISUUS))

# drop missing vessel ID's for 2013-2015
j6b |> filter(ULKOINENTUNNUS == "") |> select(ULKOINENTUNNUS, YEAR, VESSEL_LENGTH, FISHING_TECH, PRINCIPAL_SUB_REGION, ANNUAL_FISHINGDAYS, ANNUAL_FISHINGTRIPS) |> flextable::flextable()

j6b <- j6b |> filter(ULKOINENTUNNUS != "") 

# TOTGT missing values !!!!!
# missing VESSEL_LENGTH !!!!!
