...
j4 <- j3 |> group_by(YEAR, ULKOINENTUNNUS, LOMAKE, SUB_REGION) |> 
            summarise(
...
j4b <- j4 |> group_by(YEAR, ULKOINENTUNNUS, SUB_REGION) |> 
                summarise(
...
principal_sub_region <- j4b |>
  group_by(YEAR, ULKOINENTUNNUS) |>
  filter(ANNUAL_FISHINGDAYS == max(ANNUAL_FISHINGDAYS, na.rm = TRUE)) |>
...
vessel_fishing_trips <- j5 |> 
                          select(COUNTRY, YEAR, VESSEL_LENGTH, FISHING_TECH, SUPRA_REGION, GEO_INDICATOR, PRINCIPAL_SUB_REGION, ULKOINENTUNNUS, 
                                 ANNUAL_FISHINGDAYS, ANNUAL_FISHINGTRIPS) |>
                          distinct()
...
j6 <- j5 |> 
  group_by(COUNTRY, YEAR, VESSEL_LENGTH, FISHING_TECH, SUPRA_REGION, GEO_INDICATOR, PRINCIPAL_SUB_REGION) |> 
  summarise(
    TOTTRIPS = sum(ANNUAL_FISHINGTRIPS), #n_distinct(FT_REF),  changed table aggregation to fishingtrips, changed from variable fishingtrips
