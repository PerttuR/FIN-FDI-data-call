...
sas <- sas |>
  ...
  select(YEAR, ULKOINENTUNNUS=ALUS, LOMAKE, KALASTUSPAIVAT, PYYNTIPV, MERIPAIVAT, PAAKONETEHO=TEHO, # added LOMAKE
...
table_A2 |> filter(C_DISCARDS == 0) |> count() |> flextable::flextable()

# discard records with no discards
table_A2 <- table_A2 |> filter(C_DISCARDS != 0 | is.na(C_DISCARDS))
...
cat("Number of records with zero discards in table D", sep="\n")
table_A2 |> filter(D_DISCARDS == 0) |> count() |> flextable::flextable()

# discard records with no discards
table_A2 <- table_A2 |> filter(D_DISCARDS != 0 | is.na(D_DISCARDS))
...
table_A2 <- table_A2 |>
  ...
  left_join(table_E |> select(YEAR, DOMAIN_LANDINGS, SPECIES, E_xxx=xxx), 
...
table_A2 <- table_A2 |>
  #select(YEAR, DOMAIN_LANDINGS, SPECIES, TOTWGHTLANDG, TOTVALLANDG) |> # testing only
  left_join(table_F |> select(YEAR, DOMAIN_LANDINGS, SPECIES, F_TOTWGHTLANDG=TOTWGHTLANDG), 
            by=join_by(YEAR, DOMAIN_LANDINGS, SPECIES))

table_A2 |> filter(!is.na(F_TOTWGHTLANDG))
# no matches
