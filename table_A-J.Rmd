...
j_monthly <- j_monthly |>
  group_by(YEAR, ULKOINENTUNNUS, KK, SUB_REGION) |>
...
j_monthly <- j_monthly |> mutate(LOMAKE="MONTHLY") |> group_by(COUNTRY, YEAR, VESSEL_LENGTH, LOMAKE, FISHING_TECH, SUPRA_REGION,GEO_INDICATOR, FT_REF, ULKOINENTUNNUS, SUB_REGION) |> 
              summarise(KALASTUSPAIVAT = sum(KALASTUSPAIVAT, na.rm=TRUE), MERIPAIVAT = sum(KALASTUSPAIVAT, na.rm=TRUE), .groups = "drop_last") |> 
...
j_not_monthly <- j_not_monthly |> mutate(LOMAKE="TRIPS") |> group_by(COUNTRY, YEAR,VESSEL_LENGTH, LOMAKE, FISHING_TECH, SUPRA_REGION,GEO_INDICATOR,FT_REF, ULKOINENTUNNUS, SUB_REGION) |>   
                  summarise(KALASTUSPAIVAT=max(KALASTUSPAIVAT), MERIPAIVAT=max(KALASTUSPAIVAT), .groups = "drop_last") |>
                  arrange(YEAR, ULKOINENTUNNUS, FT_REF)
...
tmp <- kap |> 
  group_by(YEAR, ULKOINENTUNNUS) |> 
  filter(n()>1) |> ungroup() |> summarise(tot = n_distinct(ULKOINENTUNNUS))

cat(paste("There are", tmp$tot, "ships with multipe ship records in the same year"))

# use the first record if there is a duplicate by vessel and year
...
maxseadays_2 <- vessel_fishing_trips |>
  ...
  group_by(COUNTRY,YEAR, VESSEL_LENGTH, FISHING_TECH, SUPRA_REGION, GEO_INDICATOR, PRINCIPAL_SUB_REGION) |>
  #filter higher that 365 days?? should it be using ceiling?
...
j11 <- j10 |> mutate(FISHING_TECH = case_when(
  TOTTRIPS == 0 ~"INACTIVE",
  TRUE ~ FISHING_TECH
))
...
# Put the variables in the correct order:
table_J <- j11 |> select(COUNTRY, YEAR, VESSEL_LENGTH, FISHING_TECH, SUPRA_REGION, GEO_INDICATOR, PRINCIPAL_SUB_REGION, TOTTRIPS, TOTKW, TOTGT, TOTVES, AVGAGE, AVGLOA, MAXSEADAYS)
