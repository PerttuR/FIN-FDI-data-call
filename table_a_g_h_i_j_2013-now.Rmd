...
pacman::p_load(tidyverse, here, patchwork, RPostgres)
...
path <- here::here()
path_der  <- here::here("der", run.year)  # Location to store data
codePath  <- here::here("prog")            # Location to store R scripts
path_orig  <- here::here("orig", run.year)   # Location to store the results
plotPath  <- here::here("Plots", run.year) # Location to store plots

# Ensure outpath exists
ifelse(!dir.exists(outPath), 
          dir.create(outPath, recursive=TRUE), FALSE)

ifelse(!dir.exists(dataPath), 
          dir.create(dataPath, recursive=TRUE), FALSE)
